import requests 
from fastapi import FastAPI, HTTPException 
from pydantic import BaseModel, Field, model_validator
from qunetsim.components import Host, Network 
from qunetsim.objects import Qubit 


app=FastAPI(title="Bob service") 
class BobReq(BaseModel): 
    alice_bits:str 
    alice_basis:str 
    size:int= Field(..., ge=1, le=500)
    
    @model_validator(mode="after") 
    def check_consistency(self): 
        if len(self.alice_bits)!=self.size or len(self.alice_basis)!=self.size: 
            raise ValueError(f"alice_bits and alice_basis must have the same length as size ({self.size}") 
        return self 
    
def _get_basis(size:int)-> str: 
    try: 
        r=requests.get(f"http://localhost:8101/basis?size={size}", timeout=5) 
        r.raise_for_status() 
        return r.json()["basis"] 
    except Exception as e: 
        raise HTTPException(status_code=502, detail=f"basis_service error: {e}") 
    
def _encode(host:Host, bit:str, basis:str)-> Qubit: 
    q=Qubit(host) 
    if bit=="1": 
        q.X() 
    if basis=="X": 
        q.H() 
    return q 

def _measure(alice_bits: str, alice_basis:str, bob_basis:str) -> str: 
    results=[] 
    network=Network.get_instance() 
    alice=Host("Alice") 
    bob=Host("Bob") 
    alice.add_connection("Bob") 
    bob.add_connection("Alice") 
    network.add_hosts([alice,bob]) 
    network.start() 
    alice.start() 
    bob.start() 
    
    try: 
        for bit, alice_base, bob_base in zip(alice_bits, alice_basis, bob_basis): 
            q=_encode(alice, bit, alice_base) 
            alice.send_qubit("Bob", q, await_ack=True) 
            recv=bob.get_qubit("Alice", wait=5) 
            if recv is None: 
                raise RuntimeError("qubit lost") 
            if bob_base=="X": 
                recv.H() 
            results.append(str(recv.measure())) 
    finally: 
        network.stop(stop_hosts=True) 
    
    return "".join(results) 

@app.get("/health") 
def health(): 
    return {"statut":"ok"} 

@app.post("/bob") 
def bob(req:BobReq): 
    bob_basis=_get_basis(req.size) 
    bob_res=_measure(req.alice_bits, req.alice_basis, bob_basis) 
    return {"bob_basis":bob_basis, "bob_res": bob_res}
