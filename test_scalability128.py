"""
test_scalability_32.py
======================
Self-configuring concurrent BB84 scalability test.
...
2. Waits for all alice nodes to respond on /health.

3. Fires POST /start on all pairs simultaneously.

4. Polls KME every POLL_INTERVAL_S seconds; writes each result row to
   CSV immediately so partial results survive a crash or OOM kill.

5. Prints a summary table and exits 0 only if every session is "done".
...
SESSION_TIMEOUT_S   Hard per-session timeout       (default 1800 = 30 min)
MEMORY_WARN_GB      Warn if RAM estimate exceeds   (default 8)
KME_URL             KME base URL                   (default http://localhost:8000)
NETWORK_YAML        Path to network.yaml           (default nodes/network.yaml)

Usage
-----
# Full 32-pair run (update yaml + run test):
python test_scalability_32.py

# Quick smoke-test with 4 pairs:
MAX_PAIRS=4 python test_scalability_32.py

# Custom qubit count, 16 pairs:
MAX_PAIRS=16 N_QUBITS=512 python test_scalability_32.py
"""
...
def _pair_yaml_block(n: int) -> list[str]:
    """Return the YAML lines for one alice+bob pair."""
    alice_p, bob_p, qkdl_p = pair_ports(n)
    alice_url = f"http://localhost:{alice_p}"
...
def sync_network_yaml(n_pairs: int, yaml_path: Path) -> bool:
    """
    Write network.yaml for n_pairs if it differs from what's on disk.
    Returns True if the file was (re)written, False if it was already correct.
    """
...
async def wait_for_nodes(pairs: list[dict], timeout: float = 60.0) -> None:
    """Block until every alice node responds on /health, or raise TimeoutError."""
...
async def run_test(
    ...
    # Write failed rows immediately so they're in the CSV even if we crash
    all_rows: list[dict] = []
    for rec in failed:
...
async def main() -> int:
    ...
    # Step 1: generate / update network.yaml
    ...
    if yaml_changed:
        ...
        return 2   # distinct exit code: config changed, not a test failure
    ...
    # Step 4: build pair list from the canonical port scheme
    pairs = [pair_info(n) for n in range(1, MAX_PAIRS + 1)]

    # Step 5: open CSV before any sessions start — partial rows survive crashes
